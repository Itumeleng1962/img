...
import { Globe, Search, CheckCircle, XCircle, ArrowRight, ShieldCheck, RefreshCw, Key } from 'lucide-react';
      ...
      <section className="relative overflow-hidden pt-28 pb-20 px-6 lg:px-10 text-white"
        ...
        <div className="max-w-4xl mx-auto text-center space-y-6">
          ...
          <form onSubmit={handleSearch} className="max-w-2xl mx-auto flex gap-2">
            ...
            <button type="submit" className="bg-orange-500 hover:bg-orange-600 px-6 rounded-xl font-bold flex items-center gap-1.5 transition-colors">
              ...
              {available ? (
                <div className="flex justify-between items-center flex-wrap gap-3">
                  <div>
                    ...
                    <p className="text-xs text-blue-200 mt-1">Get it free when choosing any Annual Hosting package.</p>
                  </div>
                  <Link to={`/services/hosting/order?plan=basic&domain=${query}`} className="bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors">
                    Order Now
                  </Link>
                </div>
              ) : (
                <div>
                  ...
                  <p className="text-xs text-blue-200 mt-1">Try a different name or search with another domain extension below.</p>
                </div>
              )}
            </div>
          )}
      ...
      <section className="py-16 px-6 lg:px-10 bg-gray-50">
        <div className="max-w-4xl mx-auto space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-black text-[#0d1f3c]">Popular Domain Extensions</h2>
                ...
                <tbody className="divide-y divide-gray-100 text-gray-700">
                  {EXTENSIONS.map((e) => (
                    <tr key={e.ext} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 font-extrabold text-sm text-[#0d1f3c]">{e.ext}</td>
                      <td className="px-6 py-4 font-bold text-orange-500">{e.price}</td>
          ...
          {[
            ...
            return (
              <div key={feat.title} className="bg-gray-50 rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col items-center text-center">
                ...
                <h3 className="font-extrabold text-sm text-[#0d1f3c] mb-2">{feat.title}</h3>
